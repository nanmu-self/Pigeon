...
import { toPublicUser, type UserRow } from '../users/user.mapper.js';
...
// UserRow / toPublicUser 定义在 users/user.mapper.ts(auth 与 users 模块共用,
// 避免两处映射漂移;新增字段如 avatarUrl 只需改一处)
...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<AuthResult> {
    ...
    try {
      ...
      return { user: toPublicUser(row), token: this.signToken(row) };
    } catch (error) {
